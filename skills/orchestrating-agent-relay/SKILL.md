...
| Half-started broker: process alive but `node status` says stopped and `Failed to read broker connection metadata` | `node up` spawned a broker that never finished writing connection metadata (readiness timed out) and was not cleaned up. Do NOT just retry `node up` — it won't reap the orphan. Point **every** command in the recovery at the same state dir — carrying it on only one of them kills one broker and then starts and inspects a different one. `SD=.agentworkforce/relay` (or the `--state-dir` the broker was started with), then `agent-relay node down --force --state-dir "$SD"`, `rm -rf "$SD"`, `agent-relay node up --state-dir "$SD"`, `agent-relay node status --state-dir "$SD" --wait-for 30`. **Never `pkill -f agent-relay-broker`** — `-f` matches the full command line, so on a shared host that kills every other project's broker and the agent PTYs they own. If the orphan still will not die, read its pid from `$SD` and kill that single pid |
