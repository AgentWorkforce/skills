...
| [writing-relayflows](skills/writing-relayflows/SKILL.md) | 1.5.0 | Author a Relayflows v2 flow (`@relayflows/surface`/`@relayflows/sdk`, the `flows` CLI) in TypeScript or YAML/JSON — the run/llm/agent ladder, verification gates, cli/model resolution, flows.json, putting a local run on the Cloud dashboard with `--cloud-mirror`, and real `flows check`/`run` refusal shapes. Not the older `@relayflows/core` WorkflowBuilder engine. |
...
| [connecting-agents-across-machines](skills/connecting-agents-across-machines/SKILL.md) | 1.0.0 | Coordinate agents across two or more of your own machines — one shared workspace, a node per machine, cross-machine DMs and targeted spawns, verified agent-to-agent. |
