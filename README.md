...
Package metadata lives in [prpm.json](prpm.json). The repo currently publishes `agent-workforce-skills` version `1.6.0`.
...
| [writing-relayflows](skills/writing-relayflows/SKILL.md) | 1.3.0 | Author a Relayflows v2 flow (`@relayflows/surface`/`@relayflows/sdk`, the `flows` CLI) in TypeScript or YAML/JSON — the run/llm/agent ladder, verification gates, cli/model resolution, flows.json, and real `flows check`/`run` refusal shapes. Not the older `@relayflows/core` WorkflowBuilder engine. |
