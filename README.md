...
| [writing-relayflows](skills/writing-relayflows/SKILL.md) | 1.2.0 | Author a Relayflows v2 flow (`@relayflows/surface`/`@relayflows/sdk`, the `flows` CLI) in TypeScript or YAML/JSON — the run/llm/agent ladder, verification gates, cli/model resolution, flows.json, and real `flows check`/`run` refusal shapes. Not the older `@relayflows/core` WorkflowBuilder engine. |
...
| [relay-80-100-workflow](skills/relay-80-100-workflow/SKILL.md) | 2.0.0 | Close the 80-to-100 validation gap in a Relayflows v2 flow: the evidence recorder that turns a red check into work for a repair agent, repairable gates on the critical path, edit and hazard gates, and the fresh-eyes review rounds that catch what green gates miss. |
